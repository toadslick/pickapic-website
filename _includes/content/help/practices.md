To get the most out of Pick-a-Pic, remember these key points as you and your learner begin their functional communication journey.

### Communication must be **meaningful** and **motivating**.

Functional communication is more than just words — it is also **social behavior**. Motivation to communicate is a common barrier for individuals with autism that may need to be directly taught.

### Start with **one picture**.

How do you know what is motivating to your learner? They may show motivation by reaching, shifting their gaze, vocalizing, or using an item in a way that is fun or interesting to them. It’s important to choose an item that is actually motivating to them — not something that you think should motivate them.

Here is a real-life example of mine: I once had a learner who was more interested in playing with pipe cleaners than they were in playing with traditional toys. In that case, I had to set aside my assumption that traditional toys would be motivating to them. If I were going to create a Button Book for them in Pick-a-Pic, one of the first buttons I create would have a picture of pipe cleaners.

### Start with **requests**.

Requests establish the value of initiating and maintaining communication. For learners with limited functional communication skills, engaging in tantrums, aggression, or self-injurious behavior may serve as a way to get their needs met. Providing a functional way to communicate needs and have them met can help reduce the harmful behaviors that would otherwise serve that purpose.

### Start with **photographs**.

Photographs, as opposed to icons or illustrations, minimize any barriers related to visual discrimination. This helps draw the connection between a button being tapped and a request being fulfilled.

### **Establish yourself** as a source of reinforcement.

Ensure that the requests indicated by your Pick-a-Pic buttons are things you can quickly fulfill. This establishes the link between the communicative exchange and access to the desired items or activities.

### Be **specific** in your praise.

Provide lots of praise in a style that is motivating for your learner and that is descriptive of their behavior. Instead of saying “Great job!”, you could say “Great job showing me what you want! I love how you looked at the button!”

Make sure your style fits your learner. One learner may be startled if you raise the volume of your voice, while another learner may enjoy clapping and cheering.

Skills to look for and encourage:

- **Social approach** - body oriented towards you, movement towards you, reaching, eye contact, possibly smiling or laughing or demonstrating enjoyment.
- **Increased or purposeful vocalizations** - reinforce all attempts at vocal communication (including non-speech sounds) and any attempts at words.

### Make small, **incremental** changes.

Once your learner shows a consistent response, try varying the format in small ways. If you started with a single button on a single page, try adding a second button, and work your way toward multiple buttons on multiple pages. Have your learner try to communicate with a different person or in a different setting.

If you see an increase in challenging or harmful behavior, or your learner is not having success with a change, return to the last format where they had consistent success. Give your learner time to experience success and positive reinforcement before trying another incremental change.

### Work toward long-term goals:

The goal of **functional communication** is to establish consistent social initiation as a foundation for more complex interactions. When your learner has developed the following abilities, they may be ready to graduate from Pick-a-Pic to more advanced AAC apps:

- The ability to make requests for different categories of items, such snacks and toys.
- The ability to use buttons for more than requesting tangible objects, such as using buttons to request activities or to express feelings.
- The ability to make requests in various settings, such as outside, at the grocery store, or with other people.
- The ability to use buttons that require increased visual discrimination, such as buttons that have line drawings or emoji instead of photographs.
